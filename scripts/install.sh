#!/usr/bin/env bash
# Remote: curl -fsSL <raw-url>/scripts/install.sh | bash -s -- OWNER/REPO
...
if command -v v4l2-ctl >/dev/null; then
    if ! listing=$(v4l2-ctl --list-devices); then
        echo "Camera discovery failed. You can use 'all' to mount /dev instead." >&2
        listing=""
    fi
else
    echo "Camera discovery is unavailable. You can use 'all' to mount /dev instead."
fi
mapfile -t cameras < <(printf '%s\n' "$listing" | awk '
    /^[^ \t]/ { name=$0; found=0; next }
    /\/dev\/video[0-9]+/ && !found { print $1 "\t" name; found=1 }
')

defaults=()
echo "Detected video devices (first video node per device):"
for i in "${!cameras[@]}"; do
    entry="${cameras[i]}"
    device="${entry%%$'\t'*}"
    name="${entry#*$'\t'}"
...
main() {
    ...
    if [[ -z "$source_dir" && -z "$repo" ]]; then
        echo "Provide OWNER/REPO when running outside a source checkout." >&2
        return 1
    fi
...
if [[ ! -f "$install_dir/docker-compose.yml" ]]; then
    [[ -n "$repo" ]] || { echo "Provide OWNER/REPO to download Compose." >&2; exit 1; }
...
if ! command -v v4l2-ctl >/dev/null; then
    if command -v apt-get >/dev/null; then
        echo "Installing v4l-utils for camera discovery..."
        if ! "${sudo_cmd[@]}" apt-get update || ! "${sudo_cmd[@]}" apt-get install -y v4l-utils; then
            echo "Camera discovery installation failed; continuing with manual fallback."
        fi
    else
        echo "Camera discovery needs v4l-utils; continuing with manual fallback."
    fi
fi
configure_cameras
