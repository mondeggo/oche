...
# Emit a directory mount so Autodarts can save config atomically and retain
# companion files. Never read or print the configuration's credentials.
autodarts_config_mount() (
    config_dir="$install_home/.config/autodarts"
    if [[ -f "$config_dir/config.toml" ]]; then
        # YAML single quotes escape apostrophes by doubling them; Compose
        # escapes literal dollar signs with $$.
...
if [[ -f "$install_home/.config/autodarts/config.toml" ]]; then
    ...
    echo "Autodarts configuration: $install_dir/data/autodarts (persistent storage)."
fi
